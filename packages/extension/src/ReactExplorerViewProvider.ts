 ...
 * bundle from `dist/react-explorer/` (emitted by `packages/react-explorer`'s
 * Vite build, which also copies the icon themes into `dist/.../icons`) and
 * rewrites its asset URLs through `asWebviewUri` so the sandboxed webview can
 * resolve them.
 */
export class ReactExplorerViewProvider implements vscode.WebviewViewProvider {
  ...
  constructor(private readonly context: vscode.ExtensionContext) {
    ...
    // Icons ship inside the webview bundle (see `vite-plugin-static-copy` in
    // `packages/react-explorer/vite.config.ts`). Composing the URI off
    // `distUri` keeps a single `localResourceRoot` and avoids leaking the
    // icon layout into the extension config.
  ...
  resolveWebviewView(
    ...
    webviewView.webview.options = {
      enableScripts: true,
      // A single root covers the JS/CSS bundle, HTML, and the bundled icon
      // themes — they all live under `dist/react-explorer/`.
      ...
      async (message: IncomingMessage) => {
        ...
        try {
          ...
          if (message.type === "getRoots") {
            ...
            if (!ALLOWED_WEBVIEW_COMMANDS_SET.has(message.command)) {
              throw new Error(
    ...
    // packages/react-explorer/ (with `npm run watch:webview` running)
    // triggers a reload of this panel automatically.
...
<body>
  ...
  <p>Run <code>npm run build</code> from the repo root and reload this view.</p>
</body>
</html>`;
  }
}
